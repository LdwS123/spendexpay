...
import { triggerRailwayDeploy, pollRailwayDeployment } from "../lib/railway.js";
import { ProviderError } from "../lib/provider-error.js";

const DeployRailwayInput = z.object({
  service_id: z.string().min(1).describe("Your Railway service ID (found in the Railway dashboard → Service → Settings)"),
  environment_id: z.string().optional().describe("Your Railway environment ID (e.g. production environment UUID)"),
  mcp_token: z.string().min(1).describe("Your Spendex MCP token (starts with spx_)"),
});

// Railway charges for resource usage (CPU, RAM, egress), not per deploy trigger.
const RAILWAY_DEPLOY_COST_USD = 0;
...
export function registerDeployRailwayTool(server: McpServer) {
  server.tool(
    "deploy_to_railway",
    "[Legacy fallback — prefer pay_for_service for new integrations] Trigger a Railway service redeployment on behalf of the user. Handles payment automatically.",
    DeployRailwayInput.shape,
    async (input) => {
      if (DEV_MODE) {
        return textResponse(
          `[DEV MODE] deploy_to_railway called.\n` +
          `Service ID: ${input.service_id}\n` +
          `Environment ID: ${input.environment_id ?? "default"}\n` +
          `Token: ${input.mcp_token.slice(0, 8)}...\n\n` +
          `In production this would: charge your saved payment method, trigger a Railway redeploy, and return the dashboard URL.\n` +
      ...
      if (!user.railway_token) {
        return textResponse(
          "No Railway token found on your account. Add your Railway API token at spendexai.com/connect.",
          { isError: true }
        );
      }
      ...
      if (
        RAILWAY_DEPLOY_COST_USD > 0 &&
        user.max_auto_charge_usd > 0 &&
        RAILWAY_DEPLOY_COST_USD > user.max_auto_charge_usd
      ) {
        return {
          content: [{
            type: "text",
            text: `This action would charge $${RAILWAY_DEPLOY_COST_USD.toFixed(2)} but your auto-approve limit is $${user.max_auto_charge_usd.toFixed(2)}. Ask the user to confirm this charge or increase their limit at spendexai.com/dashboard.`,
          }],
        };
      }

      const deployDescription = `Railway deploy: ${input.service_id}${input.environment_id ? ` (env: ${input.environment_id})` : ""}`;
      ...
      const idempotencyKey = `${user.id}-railway-${input.service_id}-${Date.now()}`;
      ...
      try {
        const charge = await routePayment({
          ...
          amountUsd: RAILWAY_DEPLOY_COST_USD,
      ...
      try {
        const deployment = await triggerRailwayDeploy({
          serviceId: input.service_id,
          environmentId: input.environment_id,
          railwayToken: user.railway_token,
        });

        const polled = await pollRailwayDeployment(
        ...
        // we have not issued a refund. If RAILWAY_DEPLOY_COST_USD is ever
        ...
        const chargeNote =
          RAILWAY_DEPLOY_COST_USD > 0
            ? ` Your account was charged $${RAILWAY_DEPLOY_COST_USD.toFixed(2)} (transaction: ${txn}). Contact support at spendexai.com/support to request a refund.`
        ...
        return textResponse(
          `Railway deploy failed: ${deployMessage}.${chargeNote}${auditNote}`,
          { isError: true }
        );
      }
